import { Chat, ChatUsers, DesktopSidebar, MobileFooter } from "./utils/import";
...
import {
  ...
  setAllUsers,
  setOnlineUsers,
} from "./store/features/chatUsersSlice.js";
import socket from "./services/socketService.js";
import { ToastContainer } from "react-toastify";
import "./app.css";
import "react-toastify/dist/ReactToastify.css";

function App({ element = <ChatUsers /> }) {
      ...
      <Provider store={store}>
        <AuthContextProvider value={getUser() || null}>
          ...
          <div className="w-screen h-screen overflow-hidden bg-gray-200 flex flex-col-reverse md:flex-row ">
            ...
            {/* ChatsType Start*/}
            <div
              className={`bg-bgChattype h-[calc(100vh-40px)] md:h-full md:w-[250px] lg:w-[300px]  ${
                conversation._id ? "hidden md:block" : "block"
              }`}
            >
              {element}
            </div>
            {/* Chats End*/}
            <div
              className={`bg-bgChattype h-screen md:flex-1 z-10 md:block  ${
                conversation._id ? "block" : "hidden"
              }`}
            >
              <Chat />
            </div>
